    ...
    if file.startswith('Retina2') and file.endswith('_2.pickle'):
        ...
        for step,[img,mask] in enumerate(test_loader):
            ...
            with torch.no_grad():
                ...
                volume_sf[step,:,:] = MyFunctions.ImageRescale(x[0,2,:,:500],[0,255])
                ...
                if step % 20 == 0:
                    ...
                    plt.imshow(volume_sf[step,:,:],cmap='gray'),plt.title('self-fusion')
        ...
        # save volume
        MyFunctions.nii_saver(volume_x,root,file[:-7]+'_noi.nii.gz')
        MyFunctions.nii_saver(volume_y,root,file[:-7]+'_aver.nii.gz')
        MyFunctions.nii_saver(volume_sf,root,file[:-7]+'_sf.nii.gz')
        MyFunctions.nii_saver(volume_dn,root,file[:-7]+'_dn.nii.gz')